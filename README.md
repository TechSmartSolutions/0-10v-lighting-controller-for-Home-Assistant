...
Over the years I've searched for lighting controllers, with nothing really suiting my fancy of cheap + functional. The open-source or DIY-born options that are available for use with Home Assistant usually lack the ability to control a large number channels or drivers without buying many units, so the cost can quickly rise if you have multiple drivers.  <a href="https://www.youtube.com/watch?v=cEy5go2eZtY">A video was made for YouTube that shows how to control a MeanWell via PWM using an ESP32, optocouplers and resistors</a>.  You might find that approach suits you, but I didn't like it mainly because it 1. only works with the more expensive ESP32 instead of the cheaper and more widely available ESP8266's and 2. you also have to mess with so many little parts small parts if you have lots of drivers to control.  And if I'm being completely honest, I tried that method and it didn't function properly for my application.  In comparison, this method 1. doesn't tie up a lot of pins off the ESP (only 2 pins for 16 drivers or 3 pins for 24 drivers) which leaves many pins left for other functions, 2. I found this method to be simpler for many drivers and 3. in my case ended up being cheaper and takes up less space in the project box.  This controller being able to control up to 24 drivers from a single board means you can have lots of room to expand in a low-cost solution.
