...
A MeanWell power supply that supports external dimming will have two wires (or two terminal points) where a positive (DIM +) and negative (DIM -) wire can be connected to a dimming controller.  In the case of PWM dimming (which is the focus of this project), there will be a small current running through the wires.  When the wires are disconected from eachother, the light output will be 100% brightness.  When the wires are connected, the light output will be 0% brightness.  Our controller works off the PWM principle of the opening and closing this circuit very fast, creating the effect of either more or less current to be allowed to pass through the power supply's DIM circuit.  Although you dont need to know this in detail to build this project, you can <a href="https://www.google.com/search?q=how+does+pwm+work">read more about how PWM works.</a>
