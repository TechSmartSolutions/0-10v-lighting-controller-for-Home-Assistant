...
## Intro
This comprehensive guide is designed to help you create a versatile and cost-effective light dimming solution that integrates seamlessly with Home Assistant. With this solution, you'll be able to control most <a href="https://led.meanwell.com/productSeries.aspx">Meanwell LED drivers</a> (power supplies or ballasts) using a few electronic components. Moreover, the information provided here is also applicable to other LED drivers that use PWM dimming, so long as the driver provides source current on the dimming circuit. Although I haven't tested it yet, I think you can likely modify the setup to work as a 0-10v DC additive dimmer, which is more commonly used on LED Drivers than PWM dimming.

## Background
The control of LED grow and aquarium lights is mostly limited to dimming knobs, controllers, or other proprietary solutions offered by manufacturers or resellers. However, by building your own, cost-effective dimming controller, you can unleash a whole new level of functionality, automation, and customization. With Home Assistant, the possibilities are limited only by your imagination. From creating custom lighting profiles for different growth stages or times of day to adjusting light output based on room/water temperature, presence detection, and more.
...
Thankfully, there is an easier way. By working with the support teams at <a href="https://forums.adafruit.com/">Adafruit</a> and <a href="https://esphome.io">ESPHome</a>, a simpler, cheaper, and more versatile solution was developed. This solution avoids the need for buying pre-made controllers or messing with optocouplers, resistors, and tying up multiple pins off an ESP32 (the optocoupler method doesn't work with cheaper more commonly available ESP8266s) and instead simplifies the process into an easily scalable solution using only a few components while reducing costs.

<p align="right"><a href="#readme">...back to top</a></p>

## Applications & Use Cases
The most likely use for this controller are indoor gardens or aquarium (reef) tanks that use LED power supplies, although it can be used for home decor lighting.  The advancement of LED lighting means there are many LED drivers in use in home gardens, aquariums and even home decor lighting.  From do-it-yourself hobbyist lighting to the relative market explosion of off-the-shelf options, this controller is a nice cheap upgrade that would work with many of these systems already in use.
